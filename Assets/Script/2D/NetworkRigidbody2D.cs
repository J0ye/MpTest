...
public class NetworkRigidbody2D : NetworkBehaviour
{
    ...
    /// </summary>
    readonly ClientSyncState2D previousValue = new ClientSyncState2D();
    ...
    [SyncVar(hook = nameof(OnRigidbodyType2DChanged))]
    ...
    void SendRigidBodySettings()
    {
        ...
        if (previousValue.bodyType != target.bodyType)
        {
            CmdSendRigidbodyType2D(target.bodyType);
            previousValue.bodyType = target.bodyType;
        }
        if (previousValue.gravityScale != target.gravityScale)
        {
            CmdSendgravityScale(target.gravityScale);
    ...
    [Command]
    void CmdSendgravityScale(float gravityScale)
    {
        // Ignore messages from client if not in client authority mode
        if (!clientAuthority)
            return;
...
/// </summary>
public class ClientSyncState2D
{
    /// <summary>
